...
def run(args):
    ...
    from model.CSAM_SP import CSAM_SP
    from model.solver import Solver
    distrib.init(args)

    torch.manual_seed(args.seed)

    model = CSAM_SP(**args.csam_sp, sample_rate=args.sample_rate)
    ...
    length = int(args.segment * args.sample_rate) #length:64000
    stride = int(args.stride * args.sample_rate)  #stride:16000
    # CSAM_SP requires a specific number of samples to avoid 0 padding during training
    if hasattr(model, 'valid_length'):
