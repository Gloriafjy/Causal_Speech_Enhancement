...
from . import distrib
from .CSAM_SP import CSAM_SPStreamer
...
def add_flags(parser):
    ...
    parser.add_argument('--streaming', action="store_true",
                        help="true streaming evaluation for CSAM_SP")


parser = argparse.ArgumentParser(
        'denoiser.enhance',
        description="Speech enhancement using CSAM_SP - Generate enhanced files")
add_flags(parser)
...
def get_estimate(model, noisy, args):
    ...
    if args.streaming:
        streamer = CSAM_SPStreamer(model, dry=args.dry)
        with torch.no_grad():
            estimate = torch.cat([
